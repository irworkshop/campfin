...
raw_in <- prop_in({stt}$city_raw, vald_city)
progress %>% 
...
1. There are `r nrow({stt})` records in the database.
1. There are `r sum({stt}$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r sum({stt}$na_flag)` records missing either recipient or date.
