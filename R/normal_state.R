...
#' @param abbreviate If TRUE (default), replace state names with the 2-digit abbreviation
#' @param na A vector of values to make `NA`
#' @param na_rep If TRUE, make all single digit repeating strings `NA` (removes valid "AA")
...
normal_state <- function(state, abbreviate = TRUE, na = c(""), na_rep = FALSE, valid = NULL) {
  ...
  if (abbreviate) {
    states_df <- tibble(
      name = str_to_upper(c(state.name, "District of Columbia")),
      abb = c(state.abb, "DC")
    )
