...
#' @description To return a value of `TRUE`, (1) the first letter of `abb` must match the first
#' letter of `full`, (2) _all_ letters of `abb` must exist in `full`, and (3) those letters of
#' `abb` must be in the same order as they appear in `full`.
...
#' is_abbrev(ab = state.abb, full = state.name)
#' @export
is_abbrev <- Vectorize(
  function(abb, full) {
