...
```{r na_flag}
vtc <- vtc %>% flag_na(date, last, amount, committee)
sum(vtc$na_flag)
```
...
raw_in <- prop_in(vtc$city_raw, vald_city)
progress %>% 
...
1. There are `r nrow(vtc)` records in the database.
1. There are `r sum(vtc$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r sum(vtc$na_flag)` records missing either recipient or date.
...
write_csv(
  x = df,
  path = path(clean_dir, "vt_contribs_clean.csv"),
  na = ""
)
```
